...
export function useRecording<T=string>(moves:T[],mode:string,gameKey:number,onResume:(saved:Saved<T>)=>void,variant: 'chess'|'dai'='chess'){
 ...
 const gate=!accepted?<dialog ref={node=>{if(node&&!node.open)node.showModal();}} onCancel={event=>event.preventDefault()}><h2>棋譜の提供にご協力ください</h2><p>{variant==='dai'?'大将棋はプレイヤーが少なく、AI開発のための棋譜集めが難しいゲームです。あなたの一局をご提供いただけると助かります。':'あなたの一局を、AIの開発・改善に役立てたいと考えています。'}</p><p>提供した棋譜は非公開・期限なしで保存し、アカウント削除後も本人との紐づけを外して保持します。</p><p>プライベートでは棋譜を送信せず、この画面だけで遊べます。</p>{working&&<p role="status">接続中…</p>}{!working&&status!=='保存方針を確認して対局を開始してください。'&&<p role="status">{status}</p>}<button disabled={working} onClick={enable}>棋譜提供に協力して遊ぶ</button><button disabled={working} onClick={()=>{setPrivateMode(true);setAccepted(true);}}>プライベートで遊ぶ</button></dialog>:null;
