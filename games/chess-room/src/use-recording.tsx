...
export function useRecording<T=string>(moves:T[],mode:string,gameKey:number,onResume:(saved:Saved<T>)=>void,variant: 'chess'|'dai'='chess'){
 ...
 </details><details><summary>記録とプライバシー</summary><p>全対局の棋譜・対戦モード・結果・保存日時を、途中の盤面と「待った」前の履歴も含めて管理側に期限を設けず非公開で保存します。名前やメールは収集しません。管理者と本人のログイン済み端末が棋譜を取得できます。ログイン維持には90日間のCookieを使います。</p><p>履歴・アカウントを削除すると、棋譜とユーザーの紐づけを外し、棋譜自体は管理側に残します。他のユーザーの棋譜での学習や自己対戦は行いません。IPアドレスは棋譜DBに保存しません。通信エラー時は対局を一時停止します。以前の保存方針の棋譜は、ここで再開した場合に新しい保存方針へ移ります。</p>
 ...
 const gate=!accepted?<dialog ref={node=>{if(node&&!node.open)node.showModal();}} onCancel={event=>event.preventDefault()}><h2>棋譜の保存について</h2><p>全対局を途中の棋譜・待った前の手も含め、管理側に期限を設けず非公開で保存します。</p><p>履歴やアカウントを削除しても、本人との紐づけを外した棋譜は管理側に残ります。ログインは不要です。</p><p role="status">{status}</p><button disabled={working} onClick={enable}>同意して対局を開始</button></dialog>:null;
 return {panel,gate,model:profile.model,working,ready:recording&&!working&&savedSignature===JSON.stringify([serialized,mode,gameKey])};
}
