...
async function readBody(request:Request){
 ...
 while(true){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>16384){await reader.cancel();throw new ApiError(413,'入力が大きすぎます。');}chunks.push(value);}
...
async function profile(env:Env,id:string,registered:boolean){
 const rows=await env.DB.prepare('SELECT * FROM games WHERE player_id=? AND updated_at>? ORDER BY updated_at DESC LIMIT 50').bind(id,Date.now()-RETENTION).all<GameRow>();
...
async function sessionResponse(request:Request,env:Env,id:string,registered:boolean){
 ...
 return json(await profile(env,id,registered),200,{'Set-Cookie':cookie(request,secret)});
}
export default {
 async fetch(request:Request,env:Env):Promise<Response>{
  ...
  try{
   ...
   if(path==='/api/profile'&&request.method==='GET')return json(user?await profile(env,user.id,!!user.login_hash):{enabled:false,registered:false,model:emptyLearning(),latest:null});
   if(path==='/api/session'&&request.method==='POST'){
    const body=await readBody(request);if(body.consent!==true)throw new ApiError(400,'記録への同意が必要です。');
    if(user)return json(await profile(env,user.id,!!user.login_hash));
    const id=crypto.randomUUID();await env.DB.prepare('INSERT INTO players(id,created_at) VALUES(?,?)').bind(id,Date.now()).run();return sessionResponse(request,env,id,false);
   }
   if(path==='/api/account/login'&&request.method==='POST'){
    ...
    if(!account)throw new ApiError(401,'引き継ぎコードを確認してください。');return sessionResponse(request,env,account.id,true);
   }
   ...
   if(path==='/api/history'&&request.method==='DELETE'){
    await env.DB.prepare('DELETE FROM games WHERE player_id=?').bind(user.id).run();return json(await profile(env,user.id,!!user.login_hash));
   }
   ...
   if(path==='/api/games'&&request.method==='POST'){
    ...
    const id=crypto.randomUUID();await env.DB.prepare('INSERT INTO games(id,player_id,mode,created_at,updated_at) VALUES(?,?,?,?,?)').bind(id,user.id,body.mode,Date.now(),Date.now()).run();return json({id,revision:0});
   }
   const match=path.match(/^\/api\/games\/([a-f0-9-]{36})$/);
   if(match&&request.method==='PUT'){
    const old=await env.DB.prepare('SELECT * FROM games WHERE id=? AND player_id=?').bind(match[1],user.id).first<GameRow>();if(!old)throw new ApiError(404,'対局がありません。');
    ...
    let state;try{state=replay(body.moves);}catch{throw new ApiError(400,'合法な棋譜ではありません。');}
    ...
    return json({revision:old.revision+1,...await profile(env,user.id,!!user.login_hash)});
   }
